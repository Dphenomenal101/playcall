...
import { syncCallProcessingStatus } from "../data/processing-status.ts"
import { transcribeAudioFromUrl } from "../extraction/audio.ts"
import { createProcessingJob, updateProcessingJobStatus, type ProcessingJobType } from "./service.ts"
import { dispatchProcessingJob } from "./dispatch.ts"
import { scheduleRubricGenerationForPlaybook } from "./rubric.ts"
import { enrichAccountContext } from "../enrichment/service.ts"
import { generateStructuredObject } from "../ai/providers.ts"
...
async function processCallTranscriptionJob(job: any) {
  ...
  // Do NOT dispatch next jobs here — we're inside the Edge Function (Deno).
  // Calling dispatchProcessingJob for buyer_enrichment from within an Edge
  // Function invocation causes a nested self-call that Supabase rejects with
  // 403. The main dispatch.ts (Next.js) chains buyer_enrichment after this
  // job returns, which avoids the nesting entirely.
}
